...
type EventItem = { title: string; description: string; startDate: string; endDate?: string; displayDate?: string; url: string; image?: string; venue: string; city: string; price?: string; currency?: string; category: string; badge: string; source: string };
...
export default function Home() {
  ...
  const [newsTotalSources, setNewsTotalSources] = useState(0);
  ...
  useEffect(() => {
    fetch("/api/news?v=4").then((response) => response.ok ? response.json() : Promise.reject())
      .then((data) => { setLiveNews(data.items || []); setActiveSources(data.activeSources || 0); setNewsTotalSources(data.totalSources || 0); setUnavailableSources(data.unavailableSources || 0); })
  ...
  return (
    <main id="inicio">
      ...
      <header className="site-header">
        <a className="brand" href="#inicio" aria-label="Ir para o início"><span>Elessandro Eugênio</span><small>meu espaço</small></a>
        ...
        <button className="menu-button" type="button" aria-label={menuOpen ? "Fechar menu" : "Abrir menu"} aria-controls="main-navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}><span /><span /></button>
      </header>
      ...
      <section className="hero" id="conteudo" aria-labelledby="hero-title">
        <div className="hero-copy">
          ...
          <p>Um espaço pessoal para organizar recursos do trabalho, acompanhar notícias e encontrar programas culturais em São Vicente, na Baixada Santista e em São Paulo.</p>
        ...
        <aside className="summary-card" aria-label="Resumo do site">
          ...
          <strong>O que quero encontrar rápido</strong>
          <ul><li><span>01</span>Ferramentas que funcionam no navegador</li><li><span>02</span>Notícias com fonte e data visíveis</li><li><span>03</span>Eventos ligados às agendas oficiais</li><li><span>04</span>Conteúdo com foco regional</li></ul>
          <small>Um projeto pessoal que também pode ser compartilhado com professores e colegas do trabalho.</small>
        </aside>
      </section>
      ...
      <section className="news-section" id="noticias" aria-labelledby="news-title">
        ...
        <div className="update-note" role="status" aria-live="polite"><span className="status-dot" /> <strong>Atualização automática ativa</strong><small>{unavailableSources ? `${unavailableSources} de ${newsTotalSources || activeSources + unavailableSources} fontes não responderam agora; as demais continuam funcionando.` : "Todas as fontes responderam nesta atualização."}</small></div>
      </section>

      <section className="culture-section" id="cultura" aria-labelledby="culture-title">
        <div className="culture-intro"><div className="section-heading"><p className="eyebrow">Cultura & Eventos</p><h2 id="culture-title">Uma agenda com a minha cara.</h2><p>Atualizações automáticas da Baixada Santista, regiões próximas e São Paulo: shows, teatro, museus, exposições e experiências que valem a viagem.</p></div><div className="culture-stamp"><span>70</span><span>80</span><span>90</span><small>música · história · cultura pop</small></div></div>
        <div className="filter-row" role="group" aria-label="Filtrar eventos">{eventFilters.map((filter) => <button type="button" className={eventFilter === filter ? "active" : ""} aria-pressed={eventFilter === filter} onClick={() => setEventFilter(filter)} key={filter}>{filter}</button>)}</div>
        <div className="cultural-highlights"><div className="latest-heading"><div><p className="eyebrow">Agenda automática</p><h3>{eventFilter === "Para você" ? "Eventos encontrados" : eventFilter}</h3></div><span>{eventsUpdatedAt ? `atualizada ${formatDate(eventsUpdatedAt)}` : "atualização a cada 6 horas"}{eventSourceNames.length ? ` · ${eventSourceNames.length} fontes com eventos` : ""}</span></div>
          {eventsLoading ? <div className="news-loading">Consultando as agendas oficiais…</div> : automaticEvents.length ? <div className="events-grid">{automaticEvents.slice(0, 18).map((event) => <article className="event-card featured" key={`${event.title}-${event.startDate}-${event.venue}`}>{event.image && <img className="event-image" src={event.image} alt={`Imagem de divulgação do evento ${event.title}`} loading="lazy" referrerPolicy="no-referrer" />}<div><span className="event-type">{event.category}</span><span className="event-badge">{event.badge}</span></div><h3>{event.title}</h3>{event.description && <p>{event.description}</p>}<small>{event.displayDate || formatEventDate(event.startDate, event.endDate)} · {event.venue} · {event.city}</small>{event.price && <span className="event-price">{event.price === "0" ? "Gratuito" : event.currency ? `${event.currency} ${event.price}` : event.price}</span>}<a href={event.url} target="_blank" rel="noopener noreferrer">Ver no site oficial <span aria-hidden="true">↗</span></a></article>)}</div> : eventFilter === "Para você" ? <><div className="source-warning"><strong>Nenhum evento automático disponível neste momento.</strong><span>As agendas continuam sendo verificadas; abaixo estão destaques confirmados manualmente.</span></div><div className="events-grid">{currentHighlights.map((event) => <article className="event-card featured" key={event.title}><div><span className="event-type">{event.type}</span><span className="event-badge">{event.badge}</span></div><h3>{event.title}</h3><p>{event.text}</p><small>{event.dateLabel} · {event.place}</small><a href={event.url} target="_blank" rel="noopener noreferrer">Ver programação oficial <span aria-hidden="true">↗</span></a></article>)}</div></> : <div className="source-warning"><strong>Nenhum evento encontrado neste filtro.</strong><span>Experimente “Para você” ou consulte as agendas oficiais abaixo.</span></div>}
          {!eventsLoading && <div className="update-note"><span className="status-dot" /><strong>{eventSourceNames.length} de {eventTotalSources || 5} conectores automáticos ativos</strong><small>{eventSourceNames.join(" · ") || "Conectores em nova tentativa"}. A Sympla permanece como busca oficial manual porque bloqueia consultas automáticas sem credencial.</small></div>}</div>
      ...
      <section className="about-section" id="sobre" aria-labelledby="about-title">
        <div className="section-heading"><p className="eyebrow">Sobre o projeto</p><h2 id="about-title">Meu espaço, organizado do meu jeito.</h2><p>Criei este espaço para reunir ferramentas e informações que uso no trabalho, no sindicato e no dia a dia, com atenção especial a São Vicente e à Baixada Santista.</p></div>
        <div className="about-grid">
          <article><span>01</span><h3>Fontes identificadas</h3><p>Notícias e eventos mostram origem, data e acesso direto à publicação responsável.</p></article>
          <article><span>02</span><h3>Sem vínculo oficial</h3><p>O site não representa Prefeitura, sindicatos, veículos de imprensa ou casas de eventos.</p></article>
          <article><span>03</span><h3>Privacidade nas ferramentas</h3><p>Os arquivos da conferência de descontos são processados no seu navegador e não são enviados ao site.</p></article>
        </div>
